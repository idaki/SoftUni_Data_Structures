...
public class RoyaleArena implements IArena {
    ...
    public RoyaleArena() {
        ...
        this.dictBattleCards = new LinkedHashMap<>();
    ...
    public Iterable<Battlecard> findFirstLeastSwag(int n) {
        ...
        return this.battleCards.stream()
                .sorted(Comparator.comparingDouble((Battlecard f) -> f.getSwag()))
                .limit(n)
                .sorted(Comparator.comparingInt((Battlecard x) -> x.getId()))
    ...
    public Iterable<Battlecard> getAllByNameAndSwag() {
        ...
        for (Map.Entry<String, Set<Battlecard>> entry : cards.entrySet()) {
            battlecards.add(entry.getValue().stream().max(Comparator.comparingDouble((Battlecard b) -> b.getSwag())).orElse(null));
        }

        return battlecards.stream().filter(Objects::nonNull).sorted(Comparator.comparingDouble((Battlecard b) -> b.getSwag())).collect(Collectors.toList());
    }

    public Iterable<Battlecard> getAllInSwagRange(double lo, double hi) {
        List<Battlecard> result;
        result = this.battleCards.stream().filter((Battlecard s) -> s.getSwag() >= lo && s.getSwag() <= hi).sorted(Comparator.comparingDouble((Battlecard x) -> x.getSwag())).collect(Collectors.toList());
    ...
    public Iterable<Battlecard> getByCardType(CardType type) {
        ...
        List<Battlecard> result = this.battleTypes.get(type);
    ...
    public Iterable<Battlecard> getByCardTypeAndMaximumDamage(CardType type, double damage) {
        ...
        List<Battlecard> result = this.battleTypes.get(type).stream().filter((Battlecard v) -> v.getDamage() <= damage).sorted(Battlecard::compareTo).collect(Collectors.toList());
    ...
    public Iterable<Battlecard> getByNameAndSwagRange(String name, double lo, double hi) {
        List<Battlecard> result = this.battleCards
                .stream()
                .filter((Battlecard x) -> x.getName().equals(name) && x.getDamage() >= lo && x.getDamage() < hi)
                .sorted((Battlecard f, Battlecard s) -> Double.compare(s.getSwag(), f.getSwag()))
                .sorted(Comparator.comparingInt((Battlecard x) -> x.getId())).collect(Collectors.toList());
        if (result.size() == 0) {
            throw new UnsupportedOperationException();
        }
        return result;
    ...
    public Iterable<Battlecard> getByNameOrderedBySwagDescending(String name) {
        List<Battlecard> result = this.battleCards
                .stream()
                .filter((Battlecard x) -> x.getName().equals(name))
                .sorted(Comparator.comparingDouble((Battlecard b) -> b.getSwag()).reversed().thenComparing(Battlecard::getId)).collect(Collectors.toList());
    ...
    public Iterable<Battlecard> getByTypeAndDamageRangeOrderedByDamageThenById(CardType type, int lo, int hi) {
        ...
        List<Battlecard> result = this.battleTypes.get(type)
                .stream()
                .filter((Battlecard v) -> v.getDamage() > lo && v.getDamage() < hi)
    ...
    public void removeById(int id) {
        ...
        Battlecard toRemove = this.dictBattleCards.get(id);
